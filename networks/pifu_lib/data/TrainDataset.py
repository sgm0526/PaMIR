...
def load_trimesh_train(root_dir, subject):
    folders = os.listdir(root_dir)
    meshs = {}
...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        # self.mesh_dic = load_trimesh_train(self.OBJ, self.subjects)
    ...
    def load_sampling(self, subject):
        pamir_dataset_dir = f'/home/nas1_temp/dataset/Thuman/image_data/'
        dat_fpath = os.path.join(
            pamir_dataset_dir, subject, 'sample/samples.mat')
        try:
        ...
        samples = torch.Tensor(pts).float().permute(1,0)
        labels = torch.Tensor(pts_ov).float().permute(1,0)
    ...
    def get_color_sampling(self, subject, yid, pid=0):
        ...
        return {
            'color_samples2': samples,
            'rgbs2': rgbs_color
        }

    def load_color_sampling(self, data_item, view_id, pid=0):
        pamir_dataset_dir = f'/home/nas1_temp/dataset/Thuman/image_data/'
        uvpos_fpath = os.path.join(
            pamir_dataset_dir, data_item, 'meta/uv_pos.exr')
        uvmsk_fpath = os.path.join(
            pamir_dataset_dir, data_item, 'meta/uv_mask.png')
        uvnml_fpath = os.path.join(
            pamir_dataset_dir, data_item, 'meta/uv_nml.png')
        # uvclr_fpath = os.path.join(
        #    self.dataset_dir, constant.dataset_image_subfolder, data_item, 'color_uv/%04d.png' % view_id)
        uvclr_fpath = os.path.join(
            pamir_dataset_dir.replace('image_data', 'image_data_nolight2'), data_item,
            'color_uv_re/%04d.png' % view_id)
        try:
            uv_mask = cv2.imread(uvmsk_fpath)
            uv_mask = uv_mask[:, :, 0] != 0
            # UV render. each pixel is the color of the point.
            # [H, W, 3] 0 ~ 1 float
            uv_normal = cv2.imread(uvnml_fpath)
            uv_normal = cv2.cvtColor(uv_normal, cv2.COLOR_BGR2RGB) / 255.0
            uv_normal = 2.0 * uv_normal - 1.0
            # Position render. each pixel is the xyz coordinates of the point
            uv_pos = cv2.imread(uvpos_fpath, 2 | 4)[:, :, ::-1]

            uv_render = cv2.imread(uvclr_fpath)
            uv_render = cv2.cvtColor(uv_render, cv2.COLOR_BGR2RGB) / 255.0
        except ValueError as e:
        ...
        sample_id = np.int32(np.random.rand(self.num_sample_color) * len(surface_points))
        surface_points = surface_points[sample_id]
        surface_colors = surface_colors[sample_id]
        surface_normal = surface_normal[sample_id]

        surface_points += surface_normal * np.random.randn(self.num_sample_color, 1) * 0.01
        ...
        return {
            'color_samples': torch.Tensor(surface_points).permute(1,0),
            'rgbs': rgbs_color.permute(1,0)
        }
    ...
    def get_item(self, index):
        ...
        if self.opt.num_sample_inout:
            ...
            sample_data = self.load_sampling(subject)
            res.update(sample_data)
            # sample_data2 = self.select_sampling_method(subject)
            # res.update(sample_data2)
        ...
        if self.num_sample_color:
            color_data = self.load_color_sampling(subject, yid)
            res.update(color_data)
            # color_data2 = self.get_color_sampling(subject, yid=yid, pid=pid)
            # res.update(color_data2)
        return res
