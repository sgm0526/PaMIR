...
import util
from network.swapAE_networks import BaseNetwork
from network.swapAE_networks.stylegan2_layers import ConvLayer, ResBlock, EqualLinear
# models.networks->network.swapAE_networks
...
class PatchDiscriminator(nn.Module):
    def __init__(self ):
        ...
        channels = {
            ...
            8: min(max_nc, int(128 * channel_multiplier)),
            16: min(max_nc, int(64 * channel_multiplier)),
            32: int(32 * channel_multiplier),
            64: int(16 * channel_multiplier),
            128: int(8 * channel_multiplier),
            256: int(4 * channel_multiplier),
        ...
        pairlinear4 = EqualLinear(1024, out_dim)
        #self.pairlinear = nn.Sequential(pairlinear1, pairlinear2, pairlinear3, pairlinear4)
        self.pairlinear = nn.Sequential(pairlinear1, pairlinear3, pairlinear4)

    def forward(self, patch):
        B, C, H, W = patch.size()

        features = self.convs(patch)
        features= features.flatten(1)
        out = self.pairlinear(features)
