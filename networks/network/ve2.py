...
class VolumeEncoder_8(BaseNetwork):
    """CycleGan Encoder"""
    def __init__(self, num_in=3, num_out=32):
        super(VolumeEncoder_8, self).__init__()
        self.num_in = num_in
        self.num_out = num_out
        self.num_inter = 8
        self.relu = nn.ReLU(inplace=True)
        ...
        self.conv_inter2 = nn.Conv3d(self.num_inter, self.num_inter, bias=True, kernel_size=5,
                                     stride=2, padding=4, dilation=2)
        self.bn_inter2 = nn.GroupNorm(4, self.num_inter)
    ...
    def forward(self, x, intermediate_output=True):
        ...
        out = self.conv_inter2(out)
        out = self.bn_inter2(out)
        out = self.relu(out)
