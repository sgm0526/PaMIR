...
class TexPamirNetAttention_nerf(BaseNetwork):
    def __init__(self):
        ...
        self.add_module('mlp',
                        MLP_NeRF(256 + self.feat_ch_2D+ self.feat_ch_3D  +  self.pe_ch, self.feat_ch_occupancy,
                                 self.feat_ch_out))
    ...
    def forward(self, img, vol, pts, pts_proj, img_feat_geo, feat_occupancy, img_feat_tex=None, vol_feat=None, return_flow_feature=False):
        ...
        if return_flow_feature:
            return grid_2d_offset.squeeze(-2), volume_feature, pt_tex_att, pt_feat_3D.squeeze(), pt_tex_sigma, pt_tex_pred


        pt_tex = pt_tex_att * pt_tex_sample + (1 - pt_tex_att) * pt_tex_pred
        return pt_tex_pred, pt_tex, pt_tex_att, pt_feat_3D.squeeze(), pt_tex_sigma, None
