...
from util.base_trainer_multi import BaseTrainer
from dataloader.dataloader_tex_multi import TrainingImgDataset
from network.arch import PamirNet, TexPamirNetAttention, TexPamirNetAttention_nerf, NeuralRenderer, ResDecoder, NeuralRenderer_coord, TexPamirNetAttention_nerf_multiview
...
class Trainer(BaseTrainer):
    ...
    def train_step(self, input_batch):
        ...
        batch_size, pts_num = pts[:,0].size()[:2]
        losses = dict()
        ...
        for i in range(img.size(1)):
            ...
            sampled_points_list.append(sampled_points.unsqueeze(1))
            sampled_points_proj_list.append(sampled_points_proj.unsqueeze(1))

        sampled_points = torch.cat(sampled_points_list, 1)
        sampled_points_proj = torch.cat(sampled_points_proj_list, 1)
        ...
        if const.hierarchical:
            with torch.no_grad():
                ...
                # sign = (alphas > 0.5).int().squeeze(-1) * torch.linspace(2, 1, num_steps)[None,][None,].repeat( batch_size, num_ray, 1).to(self.device)
            ...
            nerf_output_clr_fine_,nerf_output_clr_fine,_,_, nerf_output_sigma_fine = self.pamir_tex_net.forward( img, vol, fine_points_, fine_points_proj)
